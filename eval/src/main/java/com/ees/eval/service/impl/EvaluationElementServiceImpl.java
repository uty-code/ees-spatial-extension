...
public class EvaluationElementServiceImpl implements EvaluationElementService {
    ...
    public EvaluationElementDTO createElement(EvaluationElementDTO elementDto) {
        // 1. 가중치 그룹별 합계 검증
        // 성과(PERFORMANCE)/역량(COMPETENCY)/다면평가(MULTI_DIMENSIONAL)는 각각 독립적으로 100% 가중치를 가짐
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EvaluationElementDTO> getAllElementsByPeriodId(Long periodId) {
        return elementMapper.findAllByPeriodId(periodId).stream()
                .map(this::convertToDto)
                .collect(Collectors.toList());
    }
