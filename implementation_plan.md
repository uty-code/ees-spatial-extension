...
| **평가 프로세스 혁신** | 운영지표 자동 집계 → 관리자 정성 평가 → Spatial BI 연계 분석 → 임원 최종 승인 (v2) |
...
## 8. Evaluation Logic Reform (v2)

기존의 단순 계층형 평가를 넘어, 운영 데이터와 지역 특성을 반영한 4단계 평가 체계를 도입합니다.

1. **운영지표 자동 집계 (System)**: `branch_performance_51` 데이터를 기반으로 기초 점수 자동 산출.
2. **관리자 정성 평가 (Qualitative)**: 기존 EES 역량 평가 항목 수행.
3. **Spatial BI 연계 분석 (Cross-Check)**: 지역별 운영 환경(밀집도, 상권 특성)을 고려한 **평가 보정 지표** 적용 및 이상 징후 감지.
4. **데이터 기반 최종 승인 (Executive)**: GIS 대시보드와 통합 성과 리포트를 활용한 최종 확정.
